...
ret_bg, mask_bg = cv2.threshold(gray, 254, 255, cv2.THRESH_BINARY)
...
for cnt in cnts:
    ...
    # if area > (total_area * 0.50): continue # 뭉친 세포가 제거되지 않도록 제한 해제
    hull = cv2.convexHull(cnt) # [NEW] Convex Hull로 구멍 채우기 (형태 보존 + 병합 방지)
    cv2.drawContours(binary_final, [hull], -1, 255, -1)
...
axes[1].imshow(opening, cmap='gray'); axes[1].set_title("2. Morphology Open (No Close)")
